...
from kivy.properties import ObjectProperty, NumericProperty, \
    ReferenceListProperty, BooleanProperty, ListProperty, StringProperty
...
class SwipeBehavior(object):
    ...
    delete_distance = NumericProperty(25)

    event_after_swipe = ObjectProperty(lambda x: None)
    ...
    def check_for_left(self):
        ...
        if hasattr(self.parent, 'width'):
            if (((self.x+self.width) - self.parent.x) / self.width
                            ) * 100 <= self.delete_distance:
                if self.remove_from_left:
                    self.parent.remove_widget(self)
                    self.event_after_swipe(self)
            else:
